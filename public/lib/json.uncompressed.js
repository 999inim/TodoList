...
// source from https://github.com/josephg/ShareJS
...
;
  ...
  SubDoc = (function() {
    ...
    SubDoc.prototype.del = function(pos, length, cb) {
      return this.doc.deleteAt(this.path, pos, length, cb);
    };
    ...
    SubDoc.prototype.replace = function(pos, after, cb){
      return this.doc.replaceAt(this.path, pos, after, cb);
    }
  ...
  json.api = {
    ...
    deleteAt: function(path, length, pos, cb) {
      var elem, key, op, _ref;
      ...
      if (elem[key].constructor === Array) {
        var temp=elem[key].slice();
        op.ld =temp.splice(pos, 1)[0];
      } else if (typeof elem[key] === 'string') {
        op.sd = elem[key].slice(pos, pos + length);
      }
      return this.submitOp([op], cb);
    },
    replaceAt: function(path, pos, after, cb) {
      var elem, key, op, _ref;
      ...
      if (elem[key].constructor === Array) {
        op.ld =elem[key].splice(pos, 1)[0]; //before
        op.li = after;
      } else {
        console.log("else");
      }
